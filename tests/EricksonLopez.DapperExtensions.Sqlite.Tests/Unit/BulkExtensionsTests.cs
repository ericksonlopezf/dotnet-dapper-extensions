...
public sealed class BulkExtensionsTests
{
    ...
    public async Task BulkUpsertAsync_DelegatesToBulkInsertAsync()
    {
        ...
        await connection.ExecuteAsync("CREATE TABLE items (id INTEGER PRIMARY KEY, name TEXT);");

        var parameters = new DynamicParameters();
        parameters.Add("id", 42);
        parameters.Add("name", "Upserted");

        var rows = await connection.BulkUpsertAsync("INSERT OR REPLACE INTO items (id, name) VALUES (@id, @name)", parameters);
        rows.Should().Be(1);

        var name = await connection.ExecuteScalarAsync<string>("SELECT name FROM items WHERE id = 42");
        name.Should().Be("Upserted");
    }
    ...
    public async Task BulkUpdateAsync_DelegatesToBulkInsertAsync()
    {
        ...
        var name = await connection.ExecuteScalarAsync<string>("SELECT name FROM items WHERE id = 1");
