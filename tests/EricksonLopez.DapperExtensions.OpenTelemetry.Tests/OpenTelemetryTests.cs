...
#nullable restore
    ...
    public void AddDapperOpenTelemetry_Guards_And_Registration()
    {
        ...
        // Parameterless default null action
        var defaultServices = new ServiceCollection();
        defaultServices.AddDapperOpenTelemetry();
        var defaultSp = defaultServices.BuildServiceProvider();
        var defaultOpt = defaultSp.GetRequiredService<DapperOpenTelemetryOptions>();
        defaultOpt.CaptureSqlStatements.Should().BeTrue();
    }

    [Fact]
    public async Task ExecuteAndQueryWithTelemetryAsync_ExplicitParameters_Success()
    {
        using var connection = new SqliteConnection("Data Source=:memory:");
        await connection.OpenAsync();
        ...
        await connection.ExecuteWithTelemetryAsync(
        ...
        var rows = await connection.ExecuteWithTelemetryAsync(
            "INSERT INTO products (id, name) VALUES (@Id, @Name);",
            param: new { Id = 10, Name = "Widget" },
            transaction: null,
            commandTimeout: 30,
            commandType: CommandType.Text,
            cancellationToken: cts.Token);
        ...
        var items = (await connection.QueryWithTelemetryAsync<string>(
        ...
        items[0].Should().Be("Widget");
    }

    [Fact]
    public void ResilienceRetriesCounter_RecordsMetric()
    {
        var (meterListener, metricRecords) = CreateMeterListener();
        using (meterListener)
        {
            DapperDiagnostics.ResilienceRetriesCounter.Add(3, new KeyValuePair<string, object?>("db.system", "oracle"));

            var metric = metricRecords.FirstOrDefault(r => r.InstrumentName == "db.client.resilience.retries");
            metric.Should().NotBeNull();
            metric!.Value.Should().Be(3L);
            metric.Tags["db.system"].Should().Be("oracle");
        }
    }

    [Fact]
    public async Task OpenTelemetry_WithoutActivityListener_RecordsMetricsOnSuccessAndFailure()
    {
        var (meterListener, metricRecords) = CreateMeterListener();
        using (meterListener)
        {
            ...
            // Success Execute with 0 rows
            await connection.ExecuteWithTelemetryAsync("CREATE TABLE dummy (id INT);");
            var rows0 = await connection.ExecuteWithTelemetryAsync("DELETE FROM dummy WHERE id = 999;");
            rows0.Should().Be(0);

            // Success Query
            var list = (await connection.QueryWithTelemetryAsync<int>("SELECT 1;")).ToList();
            list.Should().Equal(1);

            // Success TraceBulkOperation with 0 rows
            var bulkRows = await connection.TraceBulkOperationAsync("BulkDelete", "dummy", ct => Task.FromResult(0));
            bulkRows.Should().Be(0);

            // Failure Execute without activity
            var actExec = () => connection.ExecuteWithTelemetryAsync("INVALID EXEC SQL;");
            await actExec.Should().ThrowAsync<SqliteException>();

            // Failure Query without activity
            var actQuery = () => connection.QueryWithTelemetryAsync<int>("INVALID QUERY SQL;");
            await actQuery.Should().ThrowAsync<SqliteException>();

            // Failure TraceBulkOperation without activity
            var actBulk = () => connection.TraceBulkOperationAsync("BulkInsert", "dummy", ct => throw new InvalidOperationException("bulk fail"));
            await actBulk.Should().ThrowAsync<InvalidOperationException>();

            var errorMetrics = metricRecords.Where(r => r.InstrumentName == "db.client.commands.count" && (string?)r.Tags.GetValueOrDefault("status") == "error").ToList();
            errorMetrics.Should().HaveCount(3);
        }
    }

    [Fact]
    public async Task TraceBulkOperationAsync_Failure_CapturesExactErrorTagsAndEvents()
    {
        var (meterListener, metricRecords) = CreateMeterListener();
        using (meterListener)
        {
            ...
            var expectedEx = new ArgumentOutOfRangeException("table", "Invalid table parameter");
            var act = () => connection.TraceBulkOperationAsync("BulkUpdate", "orders", ct => throw expectedEx);
            await act.Should().ThrowAsync<ArgumentOutOfRangeException>();

            activities.Should().ContainSingle();
            var activity = activities[0];
            activity.Status.Should().Be(ActivityStatusCode.Error);
            activity.StatusDescription.Should().Be(expectedEx.Message);
            activity.GetTagItem(DapperDiagnostics.TagErrorType).Should().Be(typeof(ArgumentOutOfRangeException).FullName);
            activity.GetTagItem("db.table").Should().Be("orders");
            activity.GetTagItem(DapperDiagnostics.TagDbOperation).Should().Be("BulkUpdate");
            activity.GetTagItem(DapperDiagnostics.TagDbSystem).Should().Be("sqlite");
            activity.GetTagItem(DapperDiagnostics.TagDbName).Should().Be("main");

            var durationMetric = metricRecords.FirstOrDefault(r => r.InstrumentName == "db.client.commands.duration" && (string?)r.Tags.GetValueOrDefault(DapperDiagnostics.TagDbOperation) == "BulkUpdate");
            durationMetric.Should().NotBeNull();

            var countMetric = metricRecords.FirstOrDefault(r => r.InstrumentName == "db.client.commands.count" && (string?)r.Tags.GetValueOrDefault(DapperDiagnostics.TagDbOperation) == "BulkUpdate");
            countMetric.Should().NotBeNull();
            countMetric!.Tags["status"].Should().Be("error");
        }
    }
