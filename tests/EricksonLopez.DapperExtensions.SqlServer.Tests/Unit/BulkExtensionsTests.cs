...
public sealed class BulkExtensionsTests
{
    ...
    [Fact]
    public void CreateSqlBulkCopy_ConfiguresBulkCopyPropertiesCorrectly()
    {
        using var conn = new SqlConnection("Server=127.0.0.1;Database=TestDB;Integrated Security=true;TrustServerCertificate=true;");
        using var table = new DataTable();
        table.Columns.Add("Id", typeof(int));
        table.Columns.Add("Name", typeof(string));

        var bulkCopy = BulkExtensions.CreateSqlBulkCopy(conn, null, "dbo.TargetTable", table, batchSize: 500, bulkCopyTimeout: 120);

        bulkCopy.Should().NotBeNull();
        bulkCopy.DestinationTableName.Should().Be("dbo.TargetTable");
        bulkCopy.BatchSize.Should().Be(500);
        bulkCopy.BulkCopyTimeout.Should().Be(120);
        bulkCopy.ColumnMappings.Count.Should().Be(2);
        bulkCopy.ColumnMappings[0].SourceColumn.Should().Be("Id");
        bulkCopy.ColumnMappings[0].DestinationColumn.Should().Be("Id");
        bulkCopy.ColumnMappings[1].SourceColumn.Should().Be("Name");
    ...
    [Fact]
    public void DefaultBulkCopyExecutor_And_Writer_AreNotNull()
    {
