...
public sealed class MultiMapBuilderTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task MultiMapBuilder_Guards_ThrowOnInvalidArguments()
    {
        ...
        var actNullCompiler = async () => await builder.QueryAsync(_connection, null!);
        await actNullCompiler.Should().ThrowAsync<ArgumentNullException>().WithParameterName("compiler");

        var actNoMappings = async () => await builder.QueryAsync(_connection, _compiler);
        await actNoMappings.Should().ThrowAsync<InvalidOperationException>().WithMessage("*At least one entity mapping*");

        var actNullKeySelector = async () => await builder.QueryGroupedAsync<int>(_connection, _compiler, null!);
        await actNullKeySelector.Should().ThrowAsync<ArgumentNullException>().WithParameterName("keySelector");

        var actGroupedNoMappings = async () => await builder.QueryGroupedAsync(_connection, _compiler, o => o.Id);
        await actGroupedNoMappings.Should().ThrowAsync<InvalidOperationException>().WithMessage("*At least one entity mapping*");
    }

    [Fact]
    public async Task MultiMapBuilder_QueryFirstOrDefaultAsync_ReturnsFirstRowOrNull()
    {
        var query = Sql.Raw("SELECT 100 AS id, 50.0 AS total, 1 AS CustomerId, 'Alice' AS Name;");
        var result = await MultiMapBuilder<Order>.Query(query)
            .Map<Customer>("CustomerId", (o, c) => { o.Customer = c; return o; })
            .QueryFirstOrDefaultAsync(_connection, _compiler);
        ...
        var emptyQuery = Sql.Raw("SELECT 100 AS id, 50.0 AS total, 1 AS CustomerId, 'Alice' AS Name WHERE 1 = 0;");
        var emptyResult = await MultiMapBuilder<Order>.Query(emptyQuery)
            .Map<Customer>("CustomerId", (o, c) => { o.Customer = c; return o; })
            .QueryFirstOrDefaultAsync(_connection, _compiler);
