...
using EricksonLopez.DapperExtensions.MySql.Transactions;
using EricksonLopez.DapperExtensions.Testing.Common;
using MySqlConnector;
using Testcontainers.MySql;
using Xunit;

namespace EricksonLopez.DapperExtensions.MySql.Tests.Integration;

/// <summary>
/// Integration tests that spin up a real MySQL instance via Docker (Testcontainers).
...
public sealed class MySqlFixture : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        await connection.ExecuteAsync(BulkTestProduct.Ddl.MySqlProductsTable);
    }
...
public sealed class MySqlIntegrationTests : IClassFixture<MySqlFixture>, IAsyncLifetime
{
    ...
    public async Task ExecuteInTransactionAsync_CommitsChangesOnSuccess()
    {
        ...
        await _connection.ExecuteInTransactionAsync(async trx =>
        {
            await _connection.ExecuteAsync(
                ...
                new { Id = productId, Name = "TxProduct", Price = 39.99m, IsActive = 1 },
                trx);
        });
    ...
    public async Task ExecuteInTransactionAsync_RollsBackChangesOnException()
    {
        ...
        var act = async () => await _connection.ExecuteInTransactionAsync(async trx =>
        {
            ...
            throw new InvalidOperationException("Simulated error inside MySQL transaction");
        });
