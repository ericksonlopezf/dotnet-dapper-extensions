...
namespace EricksonLopez.DapperExtensions.Oracle.Bulk;

/// <summary>
/// Builds parameterized INSERT ALL SQL statements and Dapper parameters for bulk operations on Oracle Database.
/// </summary>
/// <remarks>
/// Oracle supports multi-row inserts via the <c>INSERT ALL INTO table VALUES (...) SELECT 1 FROM DUAL</c> syntax.
/// </remarks>
...
public sealed class BulkBuilder<T>
{
    ...
    public (string? Sql, DynamicParameters? Parameters) Build()
    {
        ...
        var intoClauses = new List<string>(_items.Count);
        ...
        for (int i = 0; i < _items.Count; i++)
        {
            ...
            intoClauses.Add($"INTO \"{_tableName}\" ({columnList}) VALUES ({string.Join(", ", colPlaceholders)})");
        }

        var sql = new StringBuilder()
            .Append("INSERT ALL ")
            .Append(string.Join(" ", intoClauses))
