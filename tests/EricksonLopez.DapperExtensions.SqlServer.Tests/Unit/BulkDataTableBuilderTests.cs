...
public sealed class BulkDataTableBuilderTests
{
    ...
    [Fact]
    public void Column_WithNullableType_HandlesNullValuesGracefully()
    {
        var products = new[]
        {
        ...
        var table = BulkDataTableBuilder.From(products)
            .Column<bool?>("flag", _ => null)
            .Column<string?>("description", _ => null)
            .Build();

        table.Rows[0]["flag"].Should().Be(DBNull.Value);
