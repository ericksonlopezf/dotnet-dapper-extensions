...
public abstract class PagedQueryExtensionsTestsBase
{
    ...
    [InlineData("")]
    [InlineData("   ")]
    public async Task QueryCursorPagedAsync_WhenCursorColumnNullOrWhiteSpace_ThrowsArgumentException(string? invalidCol)
    {
        using var connection = new TestAdoConnection();
        var parameters = new CursorPaginationParameters { First = 10 };
    ...
    [Theory]
    [InlineData("id; DROP TABLE customers;")]
    [InlineData("col name with spaces")]
    [InlineData("col--comment")]
    [InlineData("col; SELECT 1")]
    [InlineData("col' OR '1'='1")]
    [InlineData("col/*comment*/")]
    [InlineData("col$invalid")]
    [InlineData("col#hash")]
    public async Task QueryCursorPagedAsync_WhenCursorColumnContainsInvalidCharacters_ThrowsArgumentExceptionWithExpectedMessage(string invalidCol)
    {
        using var connection = new TestAdoConnection();
        var parameters = new CursorPaginationParameters { First = 10 };
        ...
        var ex = await act.Should().ThrowAsync<ArgumentException>().WithParameterName("cursorColumn");
        ex.Which.Message.Should().Contain("Invalid cursor column name.");
    }
