...
public sealed class BulkBuilderTests
{
    ...
    [Fact]
    public void Build_MultipleRows_ProducesMultipleValueRows()
    {
        var products = Enumerable.Range(1, 3)
            .Select(i => new Product(Guid.NewGuid(), $"P{i}", i * 10m, true))
            .ToList();
        ...
        sql.Should().Contain("@p0_0, @p0_1");
        sql.Should().Contain("@p1_0, @p1_1");
        sql.Should().Contain("@p2_0, @p2_1");
    }
