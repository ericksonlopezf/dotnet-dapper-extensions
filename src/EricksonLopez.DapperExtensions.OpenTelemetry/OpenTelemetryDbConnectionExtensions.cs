...
public static class OpenTelemetryDbConnectionExtensions
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            RecordSuccess(activity, Stopwatch.GetElapsedTime(startTimestamp).TotalMilliseconds, "Execute", rowsAffected);
            return rowsAffected;
        }
        catch (Exception ex)
        {
            RecordFailure(activity, Stopwatch.GetElapsedTime(startTimestamp).TotalMilliseconds, "Execute", ex);
            throw;
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            RecordSuccess(activity, Stopwatch.GetElapsedTime(startTimestamp).TotalMilliseconds, "Query", null);
            return results;
        }
        catch (Exception ex)
        {
            RecordFailure(activity, Stopwatch.GetElapsedTime(startTimestamp).TotalMilliseconds, "Query", ex);
            throw;
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            RecordFailure(activity, Stopwatch.GetElapsedTime(startTimestamp).TotalMilliseconds, operationName, ex);
            throw;
        }
    }
